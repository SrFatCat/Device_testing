...
- `Device_testing.sh` - главный файл. При запуске можно выбрать устройство или модуль. Результат работы - создание файла конфигурации
- `Testing.sh` - читает файл конфигурации и запускает процедуры тестирования
- `ATtiny13.sh` - проверка управления питанием на основе ATTiny13 через часы р/в и будильник
- `cameras.sh` - пины питания камеры
- `converter.sh` - проверка RS485/232
- `diabox.sh` - класс с диалогами
- `dry_contacts.sh` - проверка выхода СК
- `fan_switch_led.sh` - проверка работы пинов вентилятора, кнопок, светодиодов (паркоматная плата)
- `firmware.sh` - загрузка прошивки запросом файла и передачи его в скрипты прошивки flash13 и (flash которого нет)
- `gsm_mod.sh` - SIM800C
- `mega328p.sh` - проверка 328
- `pin_on_off.sh` - базовый класс
- `result.sh` - вывод результата
- `rtc.sh` - проверка часов РВ
- `sim7600.sh` - проверка 7600 ????
- 'test_atm` - бинарник SPI для 328
